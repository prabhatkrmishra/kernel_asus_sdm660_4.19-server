...
{
	struct cdfingerfp_data *cdfingerfp = g_cdfingerfp_data;
	__pm_wakeup_event(cdfingerfp->cdfinger_lock, 1000);
...
static void cdfinger_wake_lock(struct cdfingerfp_data *pdata, int arg)
{
	if (arg) {
		...
		__pm_wakeup_event(pdata->cdfinger_lock, 3000);
	}
}
...
static int cdfinger_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct fb_event *evdata = data;
	unsigned int blank;
	int retval = 0, __screen_status;
